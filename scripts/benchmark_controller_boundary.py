...
def main():
    ...
    write(out/'invocation.json',dict(implementation_source=str(source_root),seconds=args.seconds,
        probe=args.probe,human_actor=args.human_actor,observer=args.observer,physical_clock=args.physical_clock,action_period_ms=args.action_period_ms,no_render=args.no_render))
    ...
    try:
        ...
        config=dict(cli=str(CLI),cli_config=w['cli_config'],server=w['url'],database=w['database'],
            ...
            human_actor=args.human_actor,observer=args.observer,physical_clock=args.physical_clock,action_period_ms=args.action_period_ms,no_render=args.no_render)
        ...
        with (out/'probe.log').open('w') as log, (out/'process-samples.jsonl').open('w') as samples:
            ...
            until=time.monotonic()+args.seconds+300
            while probe.poll() is None:
                ...
                if values['host_available_bytes']<3*1024**3 or shutil.disk_usage(out).free<8*1024**3:
