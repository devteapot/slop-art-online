#!/usr/bin/env python3
"""ONE explicitly authorized hosted streaming generation, unedited submission to real reducers, then no further model calls."""
import argparse,hashlib,json,pathlib,shutil,subprocess,time
ROOT=pathlib.Path(__file__).resolve().parents[1]
parser=argparse.ArgumentParser(description=__doc__)
parser.add_argument('--output', default='output/carlid-luna-streaming-proof')
parser.add_argument('--config', default='configs/reasoning/codex-carlid-luna.json')
...
DB=f'sim-hosted-stream-{time.time_ns()}'
...
# Tick1 is held while the ONE explicitly authorized hosted streaming generation completes; this is not a throughput benchmark.
subprocess.run(['python3',str(ROOT/'scripts/run_carlid_npc.py'),str(OUT),'--config',str(CONFIG),'--probe-state',str(input_path)],cwd=ROOT,check=True,timeout=330)
...
manifest=dict(run=DB,db=DB,server=SERVER,scenario=s['initial'],model=config['backend']['model'],ollama='' ,reasoning=config,reasoning_version=result['metadata']['reasoning_version'],decision_format=result['metadata']['decision_format'],rules=s['version'],tick_ms=0,wasm_sha256=hashlib.sha256(WASM.read_bytes()).hexdigest(),runner_sha256=hashlib.sha256(pathlib.Path(__file__).read_bytes()).hexdigest(),generator_sha256=hashlib.sha256((ROOT/'target/debug/examples/hosted_policy_probe').read_bytes()).hexdigest(),git_head=subprocess.run(['git','rev-parse','HEAD'],cwd=ROOT,capture_output=True,text=True).stdout.strip(),cli_version=cli('--version').strip(),created_ms=int(time.time()*1000),evidence_mode='one_hosted_stream_generation_then_authority_steps',timing='one hosted streaming generation at fixed tick1, then29 direct reducer steps; no further model responses or inference')
(OUT/'manifest.json').write_text(json.dumps(manifest,indent=2))
report=dict(run=DB,ticks=s['tick'],events=len(ev),generation_calls=1,raw_output_edited=False,installed_policy_ids=[e['id'] for e in ev if e['kind']=='policy_installed'],branch_changes=[e for e in ev if e['kind']=='branch_selected'],model_results=[e['id'] for e in ev if e['kind']=='model_result'],rejections=[e for e in ev if e['kind']=='model_rejected'],metadata=result['metadata'],interpretation='Actual hosted Luna stream through the generic adapter and real authority; model timing was decoupled from logical ticks for this single-generation proof.')
