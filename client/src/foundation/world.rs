...
fn label(commands: &mut Commands, text: String, anchor: Vec3) {
    commands.spawn((
        Text::new(text.replace('·', "|").replace(['“', '”'], "\"")),
        TextFont {
            font_size: 13.,
            ..default()
        },
...
pub fn sync(
    ...
    game: Res<Game>,
    old: Query<Entity, (With<WorldEntity>, Without<Character>)>,
            ...
            for pos in -10..=10 {
                ...
                if known {
                    ...
                    if game.overlays {
                        ...
                        let age = site
                            .and_then(|s| s["observed_tick"].as_u64())
...
pub fn labels(
    ...
    mut labels: Query<(&Label, &ComputedNode, &mut Node, &mut Visibility)>,
    game: Res<Game>,
) {
    let mut occupied: Vec<Rect> = vec![];
    for (label, computed, mut node, mut visibility) in &mut labels {
        if let Ok(pos) = camera
            .0
            .world_to_viewport(&GlobalTransform::from(*camera.1), label.anchor)
        {
