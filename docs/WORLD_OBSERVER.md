...
## Persistent world labels — 2026-09-05

World labels retain their Bevy entities across subscribed snapshots, updating text only when its content changes. Actor callouts follow the interpolated sprite transform each rendered frame. Stable actor/site ordering resolves overlaps, and `UiTransform` translates the cached layout instead of changing layout coordinates each frame. Bevy text already uses glyph texture atlases; recreating hidden, unmeasured labels on each snapshot caused the visible gaps. Selection rings now remain children of their sprites. Scenery rebuilds only when the run, visibility, or displayed site resources/hazards change. The inspector and other HUD panels retain their existing refresh path.

The focused client regression test passes 100 time-only snapshots while retaining label/scenery entities and label visibility, then checks changed text and hidden-world cleanup. Native checking and the browser WASM build pass. Chromium exercised a separate 60-second authored fixture with no inference, plus label toggling and camera pan, with no browser errors. Screenshots are under `output/label-render-verification/labels-{live,off,restored}.png`. This verifies rendering behavior, without establishing a frame-rate benchmark.
