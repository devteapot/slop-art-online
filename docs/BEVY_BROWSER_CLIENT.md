...
Current `m1-5` host details are in [Participant agents](PARTICIPANT_AGENTS.md). Its default is port 18891 and output `output/participant-agent-dev`; the preserved m1-4 host on 18890 and verification below remain historical evidence. The current built-in harness has replaced the older host Reasoner dispatch.
